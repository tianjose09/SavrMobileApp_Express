...
export default function DonorRegistration({ navigation }: any) {
  ...
  const handleRegister = async () => {
    ...
    if (donorType === 'organization' && !form.website_url) {
    ...
    setIsLoading(true);
    try {
      const name = donorType === 'individual' ? form.first_name : form.organization_name;
      await ApiService.sendVerificationEmail({ email: form.email, name });

      const role = donorType === 'individual' ? 'donor' : 'organization';
      const targetDashboard = donorType === 'individual' ? 'DonorDashboard' : 'OrgDashboard';
                ...
                <>
                      ...
                      <View style={styles.inputBlock}>
                        ...
                        <View style={[styles.lineInputWrap, styles.pickerContainer]}>
                          <CustomDropdown
                            ...
                            placeholder={isLoadingProvinces ? "Loading..." : "Select Region"}
                            items={provinces}
                  ...
                  <View style={styles.addressRow}>
                    ...
                    <View style={[styles.addressCol, { flex: 1.7 }]}>
                      {renderLineInput('Postal / ZIP Code', 'postal_zip_code', { keyboardType: 'numeric', isRequired: true })}
                    </View>
                  </View>
                </>
              ) : (
