...
import {
  ActivityIndicator, Alert, Image, LayoutAnimation, Modal, Platform, ScrollView, StatusBar, StyleSheet, Text, TextInput, TouchableOpacity, UIManager, View
} from 'react-native';
...
export default function TrackMyRequest({ route, navigation }: any) {
  ...
  const handleSubmitFinancialReceipt = async (requestId: number, disbursementId?: number | string | null) => {
    try {
      let res: any;
      if (disbursementId) {
        // @ts-ignore
        res = await ApiService.confirmDisbursement(requestId, disbursementId);
      } else {
        res = await ApiService.confirmFinancialReceived(requestId);
      }
      if (res?.data?.success) {
