...
export const ApiService = {
  ...
  declineEarly: (id: number) => api.post(`api/donation/pickup/${id}/decline-early`),
  confirmDelivery: (id: number, note?: string, tracking_link?: string) => api.put(`api/donation/pickup/${id}/delivered`, { note: note || null, tracking_link: tracking_link || null }),
  ...
  confirmFinancialReceived: (id: number) => api.post(`api/donation/my-requests/${id}/financial-received`),
  ...
  updateMealRequestStatus: (id: number, status: string) =>
    api.patch(`api/partner-kitchen/meal-requests/${id}/status`, { status }),
